...
const sumArray = (array) => {
    ...
    // recorrer el array e ir sumando cada elemento
...
const calcularDuracionTotal = (peliculas) => {
     return peliculas.reduce((total, movie) => {
    if (movie.duracion > 0) {
      return total + movie.duracion;
    } else {
      return total;
    }
  }, 0);
...
const calcularPromedioPuntajes = (peliculas) => {
  ...
  const scores = peliculas.reduce((total, movie) => {
    if (movie.puntaje && movie.puntaje >= 0) {
      total.count++;
      total.total += movie.puntaje;
    }
    return total;
  }, { count: 0, total: 0 });
  if (scores.count === 0) {
...
const calcularPromedioDuracionPorGenero = (peliculas, genero) => {
  ...
  const totalDuration = moviesByGenre.reduce((acc, movie) => {
    return acc + movie.duracion;
  }, 0);

  const averageDuration = totalDuration / moviesByGenre.length;
